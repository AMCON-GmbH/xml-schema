    ...
    fn implement(
        ...
        let (fields, extra_structs) = if let Some(kind) = &self.kind {
          ...
          pub content: xml_schema_types::#extern_type,
            ...
            let fields_definition = self.complex_type.iter().map(|complex_type| complex_type.get_field_implementation(context, prefix)).collect();
...
impl Element {
    ...
    pub fn get_field_implementation(
        ...
        let multiple = self.max_occurences.is_some() && self.max_occurences != Some(MaxOccurences::Number { value: 1 });
        ...
        let name = if self.name.to_lowercase() == "type" {
            ...
            refers.expect("[Element] refers should be defined").to_snake_case()
        };
        ...
        let module = (!context.is_in_sub_module() && !self.kind.as_ref().map(|kind| {
            RustTypesMapping::is_xs_string(context, kind) || RustTypesMapping::is_xs_int(context, kind)
        }).unwrap_or_default()).then_some(quote!(xml_schema_types::)).unwrap_or_default();
        ...
        let prefix_attribute = prefix.as_ref().map(|prefix| quote!(, prefix=#prefix)).unwrap_or_default();
...
mod tests {
    ...
    static DERIVES: &str = "#[derive(Clone, Debug, Default, PartialEq, yaserde_derive::YaDeserialize, yaserde_derive::YaSerialize)]";
